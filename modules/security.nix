  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf cfg.fingerprint.enable {
      ...
      services.udev.extraRules = ''
        ACTION=="add", SUBSYSTEM=="usb", ATTR{idVendor}=="06cb", ATTR{idProduct}=="00f0", ATTR{power/control}="on", ATTR{power/autosuspend}="-1"
      '';

      powerManagement.powerDownCommands = ''
        ${pkgs.systemd}/bin/systemctl stop fprint.service 2>/dev/null || true
      '';
